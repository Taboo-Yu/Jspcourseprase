						...
						<ul class="slides">
							<li data-thumb="${imgUrl}/goods/${goods.goodsImg}">
								<div class="thumb-image">
									<img src="${basePath}image/${goods.goodsImg}"
								...
								<c:forEach items="${goodsImgList}" var="item">
									<c:if test="${item.goodsImgType == 0}">
										...
										<div class="thumb-image"><img
											src="${basePath}image/${item.goodsImgUrl}" data-imagezoom="true" class="img-responsive">
											</div></li>
									</c:if>
								</c:forEach>
							</c:if>
				...
				<c:if test="${goodsImgList != null && fn:length(goodsImgList) > 0}">
					<c:forEach items="${goodsImgList}" var="item">
						<c:if test="${item.goodsImgType == 1}">
							...
							<img  class="img-responsive" src="${basePath}image/${item.goodsImgUrl}">
							</div>
						</c:if>
					</c:forEach>
				</c:if>
