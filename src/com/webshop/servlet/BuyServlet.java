...
public class BuyServlet extends BaseServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (address == null) {//没有则插入
			address = new Address();
			address.setAddressId(UUID.randomUUID().toString());//UUID唯一标识码
			address.setAddress(add);
			address.setUserId(user.getUserId());
			res1 = addressService.insertAddress(address);
		}
		//插入订单，插入订单子表，从购物车中删除
		Order order = new Order(UUID.randomUUID().toString(), user.getUserId(), new Date(), address.getAddressId());//生成随机唯一订单码
